...
class optional
{
   public:
	...
	optional& operator=(const T& value)
	{
		if (is_initialized_)
		{
			this->value() = value;	// Присваиваем значение, если optional уже
									// инициализирован
		}
		else
		{
	...
	optional& operator=(T&& value)
	{
		...
		{
			new (&data_[0])
				T(std::move(value));  // Создаем значение с перемещением
			is_initialized_ = true;
		}
		return *this;
	}
	...
	optional& operator=(optional&& other) noexcept
	{
		...
		{  // Защита от самоприсваивания
			if (other.is_initialized_)
			{
				...
				{
					new (&data_[0])
						T(std::move(other.value()));  // Создаем с перемещением
					is_initialized_ = true;
				}
				// Помечаем исходный объект как пустой после перемещения
				other.is_initialized_ = false;
	...
	T& value() &
	{
		if (!is_initialized_)
		{
			throw bad_optional_access();  // Бросаем исключение если optional
										  // пуст
		}
	...
	void emplace(Args&&... args)
	{
		...
		new (&data_[0])
			T(std::forward<Args>(args)...);	 // Создаем новое значение
		is_initialized_ = true;
	}
